...
class PaymentNotificationType(str, enum.Enum):
...
class Lot(Base):
    ...
    suspected_airbnb = Column(Boolean, nullable=True, default=False)
    renting_locker = Column(Boolean, nullable=True, default=False)
    locker_number = Column(Text, nullable=True)
    locker_signup_date = Column(Date, nullable=True)
    notes = Column(Text, nullable=True)
    search_vector = Column(TSVECTOR, nullable=True)
    created_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now())
    updated_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now())
...
class PaymentConfig(Base):
    """Singleton row (id=1) for global payment notification settings."""
    __tablename__ = "payment_config"

    id = Column(Integer, primary_key=True, default=1)
    advance_notice_days = Column(Integer, nullable=False, default=45)
    additional_reminder_days = Column(postgresql.ARRAY(Integer), nullable=False, default=lambda: [30, 14, 7])
    past_due_notice_days = Column(postgresql.ARRAY(Integer), nullable=False, default=lambda: [1, 7, 14, 30])
    ...
    __table_args__ = (
        CheckConstraint("id = 1", name="ck_payment_config_singleton"),
    )


class PaymentSchedule(Base):
    """Recurring payment obligation (e.g. locker rental)."""
    ...
    frequency = Column(SAEnum(PaymentFrequency, name="paymentfrequency"), nullable=False, default=PaymentFrequency.monthly)
    ...
    payments = relationship(
        "Payment", back_populates="payment_schedule", cascade="all, delete-orphan",
        order_by="Payment.due_date",
    )

    __table_args__ = (
        Index("ix_payment_schedules_lot", "lot_id"),
        Index("ix_payment_schedules_party", "party_id"),
        Index("ix_payment_schedules_active", "is_active"),
    )


class Payment(Base):
    """Individual payment record — generated by scheduler or recorded manually."""
    ...
    status = Column(SAEnum(PaymentStatus, name="paymentstatus"), nullable=False, default=PaymentStatus.pending)
    payment_method = Column(SAEnum(PaymentMethod, name="paymentmethod"), nullable=True)
    reference_number = Column(String(100), nullable=True)
    notes = Column(Text, nullable=True)
    created_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now())
    updated_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now())

    payment_schedule = relationship("PaymentSchedule", back_populates="payments")
    lot = relationship("Lot", foreign_keys=[lot_id])
    party = relationship("Party", foreign_keys=[party_id])
    notifications = relationship(
        "PaymentNotification", back_populates="payment", cascade="all, delete-orphan"
    )

    __table_args__ = (
        Index("ix_payments_schedule", "payment_schedule_id"),
        Index("ix_payments_lot", "lot_id"),
        Index("ix_payments_party", "party_id"),
...
class PaymentNotification(Base):
    """Audit log of outbound payment notifications."""
    __tablename__ = "payment_notifications"

    id = Column(Integer, primary_key=True)
    payment_id = Column(Integer, ForeignKey("payments.id", ondelete="CASCADE"), nullable=False)
    notification_type = Column(SAEnum(PaymentNotificationType, name="paymentnotificationtype"), nullable=False)
