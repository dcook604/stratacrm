...
export type ImportBatchStatus = "pending" | "reviewing" | "completed" | "cancelled";
export type StagedLotStatus = "pending" | "confirmed" | "skipped";
export type DuplicateConfidence = "none" | "low" | "medium" | "high";
export type StagedPartyAction = "create" | "merge" | "skip";
...
export interface ImportBatch {
  ...
  total_lots: number;
  lots_confirmed: number;
  lots_skipped: number;
  lots_pending: number;
}

export interface StagedContactMethod {
  method_type: ContactMethodType;
  value: string;
  is_primary: boolean;
}
...
export interface StagedParty {
  ...
  mailing_province: string | null;
  mailing_postal_code: string | null;
  contact_methods: StagedContactMethod[];
  form_k_filed_date: string | null;
  notes: string | null;
  detected_duplicate_party_id: number | null;
  duplicate_confidence: DuplicateConfidence;
  duplicate_party_name: string | null;
  action: StagedPartyAction | null;
  merge_target_party_id: number | null;
}
...
export interface PaginatedStagedLots {
  items: StagedLot[];
  total: number;
  lots_pending: number;
  lots_confirmed: number;
  lots_skipped: number;
  lots_with_issues: number;
}

export interface ConfirmLotResponse {
  created: number;
  merged: number;
  skipped: number;
...
export interface BatchDiff {
  changed_lots: {
    staged_lot_id: number;
    strata_lot_number: number;
    unit_number: string | null;
    new: { full_name: string; role: string }[];
    departed: { party_id: number; full_name: string }[];
  }[];
...
export const importApi = {
  upload: async (file: File): Promise<ImportBatch> => {
    const form = new FormData();
    form.append("file", file);
  ...
  getBatch: (id: number) => api.get<ImportBatch>(`/import/batches/${id}`),
  listLots: (batchId: number, params: { skip?: number; limit?: number; issues_only?: boolean; status?: string }) => {
    const qs = new URLSearchParams();
    if (params.skip != null) qs.set("skip", String(params.skip));
    if (params.limit != null) qs.set("limit", String(params.limit));
    if (params.issues_only) qs.set("issues_only", "true");
    if (params.status) qs.set("status", params.status);
    return api.get<PaginatedStagedLots>(`/import/batches/${batchId}/lots?${qs}`);
  },
  getLot: (batchId: number, lotId: number) =>
    api.get<StagedLot>(`/import/batches/${batchId}/lots/${lotId}`),
  setPartyAction: (batchId: number, lotId: number, partyId: number, body: { action: StagedPartyAction; merge_target_party_id?: number }) =>
    api.patch(`/import/batches/${batchId}/lots/${lotId}/parties/${partyId}`, body),
  confirmLot: (batchId: number, lotId: number) =>
    api.post<ConfirmLotResponse>(`/import/batches/${batchId}/lots/${lotId}/confirm`),
  skipLot: (batchId: number, lotId: number) =>
    api.post(`/import/batches/${batchId}/lots/${lotId}/skip`),
  getDiff: (batchId: number) =>
    api.get<BatchDiff>(`/import/batches/${batchId}/diff`),
};
